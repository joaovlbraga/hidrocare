...
from app.schemas import PatientCreate, PatientPublic
...
def list_patients(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return db.scalars(
        select(Patient)
        .where(Patient.is_admitted.is_(True), Patient.is_active.is_(True))
        .order_by(Patient.bed, Patient.full_name)
    ).all()
